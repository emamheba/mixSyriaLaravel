...
@section('site-title')
    {{__('User Membership Inactive Email Template')}}
@endsection
@section('style')
    <x-media.css/>
    <x-summernote.css/>
            ...
            <div class="dashboard__card bg__white padding-20 radius-10">
                <div class="dashboard_orderDetails__header__flex">
                    <div class="dashboard_orderDetails__header__left">
                         <h2 class="dashboard__card__header__title mb-3">{{__('User Membership Inactive Template')}}</h2>
                    </div>
                ...
                <x-validation.error/>
                <form action="{{route('admin.email.user.membership.inactive.template')}}" method="POST">
                    @csrf
                    <div class="form__input__single">
                        <label for="user_membership_inactive_email_subject" class="form__input__single__label">{{__('Email Subject')}}</label>
                        <input type="text" class="form__control radius-5" name="user_membership_inactive_email_subject" value="{{get_static_option('user_membership_inactive_email_subject') ?? 'User Membership Inactive Email'}}">
                    </div>
                    <div class="form__input__single">
                        <label for="user_membership_inactive_message" class="form__input__single__label">{{__('User Membership Inactive Message')}}</label>
                        <textarea class="form__control summernote" name="user_membership_manual_payment_complete_message">{!! get_static_option('user_membership_inactive_message') ?? 'Your membership status changed from active to inactive. Membership ID: @membership_id'  !!} </textarea>
                    </div>
                    <div class="d-grid">
