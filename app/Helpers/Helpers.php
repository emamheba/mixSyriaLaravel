...
{
  if (empty($id)) {
    $noImageMarkupRender = asset('storage/uploads/no-image.png');
...
{
  $noImageMarkup = 'style="background-image: url(' . asset('storage/uploads/no-image.png') . ');"';
  if (empty($id))
...
function get_attachment_image_by_id($id, $size = null, $default = false)
{
  ...
  if (file_exists('storage/uploads/media-uploader/' . optional($image_details)->path)) {
    $image_url = asset('storage/uploads/media-uploader/' . optional($image_details)->path);
  }

  if (!empty($id) && !empty($image_details)) {
    switch ($size) {
      case "large":
        if (file_exists('storage/uploads/media-uploader/large/' . $image_details->path)) {
          $image_url = asset('storage/uploads/media-uploader/large/' . $image_details->path);
        }
        break;
      case "grid":
        if (file_exists('storage/uploads/media-uploader/grid/' . $image_details->path)) {
          $image_url = asset('storage/uploads/media-uploader/grid/' . $image_details->path);
        }
        break;

      case "semi-large":
        if (file_exists('storage/uploads/media-uploader/semi-large/' . $image_details->path)) {
          $image_url = asset('storage/uploads/media-uploader/semi-large/' . $image_details->path);
        }
        break;
      case "square_120":
        if (file_exists('storage/uploads/media-uploader/square_120/' . $image_details->path)) {
          $image_url = asset('storage/uploads/media-uploader/square_120/' . $image_details->path);
        }
        break;
      case "square_80":
        if (file_exists('storage/uploads/media-uploader/square_80/' . $image_details->path)) {
          $image_url = asset('storage/uploads/media-uploader/square_80/' . $image_details->path);
        }
        break;
      case "thumb":
        if (file_exists('storage/uploads/media-uploader/thumb/' . $image_details->path)) {
          $image_url = asset('storage/uploads/media-uploader/thumb/' . $image_details->path);
        } else {
          if (file_exists('storage/uploads/media-uploader/' . $image_details->path)) {
            $image_url = asset('storage/uploads/media-uploader/' . $image_details->path);
          }
        }
        break;
      default:
  ...
  if (!empty($image_details)) {
    ...
    $return_val['img_url'] = asset('storage/uploads/no-image.png');
  }
...
{
  $image_url = asset('frontend/img/no-image-bg.png') ?? '';
...
function userProfileImageView($user_image_id)
{
  ...
  if (!empty($user_image_id)) {
    ...
    $output = '<img src="' . asset('frontend/img/static/user-no-image.webp') . '" alt="No Image" width="48px" height="48px">';
  }
  return $output;
}
