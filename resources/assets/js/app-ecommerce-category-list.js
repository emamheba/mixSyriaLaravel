...
$(function () {
    ...
    if (dt_category_list_table.length) {
      var dt_category = dt_category_list_table.DataTable({
        ajax: {
          url: '/api/categories',  // Call the new API route to fetch categories
          dataSrc: function (json) {
            ...
            return json.map(function (category) {
              return {
                ...
                total_products: category.listings.length,
                total_earnings: '$' + category.listings.reduce(function (sum, listing) {
                  return sum + listing.price; // assuming each listing has a `price` field
                }, 0),
                category_detail: category.description,
                cat_image: category.image || 'default-image.jpg'
              };
            });
          }
        },
